...
####
print("CODE IN PYTHON 3.11 TO CREATE TEXT FOR  WIKIPEDIA ARTICLE FOR A VILLAGE IN MAHARASHTRA BASED ON CENSUS 2011 ")
print('On running the program the text will appear in folder "census_data"; open the file in notepad and use it as a wiki article.')

try:
   df = pd.read_csv('census_data/mah_vill_census_data.csv',low_memory=False)  # read the csv file into a DataFrame df
...
code = input("Enter village code as is given in census 2011 data : ")
print("code=", code)
...
try:
    with open('census_data/e_m_list_' + dist_name + '.pkl', 'rb') as fd_dist:
    ...
    print('Village names in Devnagari script for this district not available : Some names will not appear in Devnagari :')
...
while i < 1:  # tot_rows-1 :
    ...
    f1 = open('census_data/wikipage_text.txt', 'w', encoding='utf-8')
